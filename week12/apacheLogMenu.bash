...
function frequentVisitors(){
    while read -r count IP withoutHours; do
        if [ "$count" -gt 10 ]; then
            printf "%s %s %s\n" "$count" "$IP" "$withoutHours"
        fi
    done < <(histogram)
}
...
function suspiciousVisitors(){
	...
	tail access.txt | egrep -i -f ioc.txt
}
...
do
	...
	# Suspicious visitors
	echo "[6] Display Suspicous Activity"
	echo "[7] Quit"
